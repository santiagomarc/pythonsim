...
from benchmarks import run_performance_sweep, _sample_cpu_utilization
...
def parse_args():
    ...
    parser.add_argument("--patients", type=int, default=500, help="Number of patients to run in the single trial")
    ...
    parser.add_argument("--verbose", action="store_true", help="Print timestamped logs per event (e.g. arrivals, departures)")
...
def main():
    ...
    print(f"  • Single Trial Target: {args.patients} patients")
    ...
    results, exec_time = run_simulation_suite(
        n_replications=1,
        n_workers=1,
        lam=args.lam,
    ...
    step1_cpu = psutil.cpu_percent(interval=None) # Capture the CPU usage over that duration
    ...
    # ----------------------------------------------------
    # Step 3: Input Distribution Statistical Validation (pooled from N={n} runs)
    # ----------------------------------------------------
    print("\n[Step 3/5] Performing input distribution goodness-of-fit validation...")
    all_interarrivals = []
    all_services = []
    for r in convergence_data[n]:
    ...
    with open(report_filename, "w", encoding="utf-8") as report_file:
        ...
        log(f"  - Average Queue Wait Time: {summ['avg_wait_time']['mean']:.4f} min (95% CI: [{summ['avg_wait_time']['ci_95'][0]:.4f}, {summ['avg_wait_time']['ci_95'][1]:.4f}], CV: {summ['avg_wait_time']['cv']:.4f})")
        log(f"  - Job Throughput:          {summ['throughput']['mean']:.4f} jobs/min (95% CI: [{summ['throughput']['ci_95'][0]:.4f}, {summ['throughput']['ci_95'][1]:.4f}], CV: {summ['throughput']['cv']:.4f})")
        log(f"  - CPU Utilization:         {summ['server_utilization']['mean'] * 100:.2f}% (95% CI: [{summ['server_utilization']['ci_95'][0]*100:.2f}%, {summ['server_utilization']['ci_95'][1]*100:.2f}%])")
        log("")
        log("======================================================================")
        log("3. OVERALL SYSTEM SUMMARY")
        log("----------------------------------------------------------------------")
        log(f"- Total Patients:       {args.patients}")
        log(f"- Total Execution Time: {exec_time:.4f} seconds")
        log(f"- Average Waiting Time: {queueing_data[n].avg_wait_time:.4f} min")
        log(f"- Number of Counters:   {args.c}")
        tp_n_val = queueing_data[n].total_patients_served / queueing_data[n].sim_duration
        log(f"- Job Throughput:       {tp_n_val:.4f} jobs/min")
        log(f"- CPU Utilization:      {step1_cpu:.1f}%")
        log("======================================================================")
    ...
    print("├──────────────────────────────┬─────────────────────────────────────────┤")
    print(f"│ Patient Count (N)            │ {n:^39} │")
    print("├──────────────────────────────┼─────────────────────────────────────────┤")
    print(f"│ Average Queue Wait Time (min)│ {queueing_data[n].avg_wait_time:^39.2f} │")
    print(f"│ Avg Queue Length             │ {queueing_data[n].avg_queue_length:^39.2f} │")
    print(f"│ CPU Utilization              │ {queueing_data[n].server_utilization*100:^38.2f}% │")
    # Throughput: patients served per simulation minute
    tp_n = queueing_data[n].total_patients_served / queueing_data[n].sim_duration
    print(f"│ Job Throughput (jobs/min)    │ {tp_n:^39.2f} │")
    print("└──────────────────────────────┴─────────────────────────────────────────┘")
    
    # Table 2: Performance Timing Comparison Table
    print("\n┌──────────────────────────────────────────────────────────────────────────────────────┐")
    print(f"│ ⚙️ PERFORMANCE COMPARISON SWEEP (Replications = {args.replications:^2} per count)                         │")
    print("├──────────────┬───────────────┬────────────┬──────────┬──────────────┬──────┬──────┤")
